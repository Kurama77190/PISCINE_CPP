...
/*   Updated: 2025/04/16 15:37:54 by sben-tay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Form.hpp"

Form::Form( std::string name, bool isSigned, int toSigne, int toExecute)
	:	_name(name), _isSigned(isSigned), _requiredGradeToSign(toSigne),
		_requiredGradeToExecute(toExecute)
{
...
{
	o << "Form name: " << i.getName() << ", is signed: " << i.getIsSigned() << ", required grade to sign: "
	<< i.getRequiredGradeToSign() << ", required grade to execute: "<< i.getRequiredGradeToExecute() << std::endl;
	return (o);
}
