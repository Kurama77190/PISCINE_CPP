...
/*   Updated: 2025/04/15 17:35:05 by sben-tay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
    ...
    *defaultBureaucrat = *paramBureaucrat; // <----- deep copy
    ...
    std::cout << GREEN << "===========TEST FORM SIGNATURE=========== " << RESET << std::endl;
    ...
    std::cout << GREEN << "===========TEST FORM EXCEPTIONS=========== " << RESET << std::endl;

    try {
        Form invalidFormHigh("InvalidFormHigh", false, 0, 20); // Invalid grade (too high)
    } catch (std::exception &e) {
        std::cerr << RED << e.what() << RESET << std::endl;
    }

    try {
        Form invalidFormLow("InvalidFormLow", false, 151, 20); // Invalid grade (too low)
    } catch (std::exception &e) {
        std::cerr << RED << e.what() << RESET << std::endl;
    }
